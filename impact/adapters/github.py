...
from impact.domain.models import (
    ...
    FileRecord, TimelineEvent, PullRequestState, ReviewState, CommentType, UserType
)
...
class GitHubAdapter(ProviderAdapter):
    def parse_dump(self, dump_path: str) -> CanonicalBundle:
        ...
        # Parse pull_requests.jsonl (collect raw, build later)
        pr_raw = []
        pr_file = path / "canonical" / "pull_requests.jsonl"
        if pr_file.exists():
            with open(pr_file, 'r') as f:
                for line in f:
                    pr_raw.append(json.loads(line.strip()))
        raw_pr_numbers = {pr['number'] for pr in pr_raw}

        # Parse reviews.jsonl
        reviewed_pr_numbers = set()
        ...
        if review_file.exists():
            with open(review_file, 'r') as f:
                for line in f:
                    ...
                    if user_dict['login'] == user_login or pr_number in raw_pr_numbers:
                        ...
                        reviewed_pr_numbers.add(pr_number)

        # Build PR objects after knowing reviewed PRs
        for pr_dict in pr_raw:
            pr_number = pr_dict['number']
            user_dict = pr_dict['user']
            if user_dict['login'] == user_login or pr_number in reviewed_pr_numbers:
                user_id = user_dict['id']
                if user_id not in users:
                    users[user_id] = User(**user_dict)
                user = users[user_id]
                ...
                if repo_id not in repos:
                    ...
                    owner_dict.setdefault('type', 'Organization')
        ...
        # Parse commits.jsonl (after knowing reviewed PRs)
        ...
        if commit_file.exists():
            with open(commit_file, 'r') as f:
                for line in f:
                    ...
                    if author_dict['login'] == user_login or pr_number in user_pr_numbers or pr_number in reviewed_pr_numbers:
                        commit = Commit(
                            sha=commit_dict['sha'],
        ...
        if rc_file.exists():
            with open(rc_file, 'r') as f:
                for line in f:
                    ...
                    if user_dict['login'] == user_login or pr_number in user_pr_numbers or pr_number in reviewed_pr_numbers:
                        user_id = user_dict['id']
                        if user_id not in users:
                            users[user_id] = User(**user_dict)
                        user = users[user_id]
        ...
        if ic_file.exists():
            with open(ic_file, 'r') as f:
                for line in f:
                    ...
                    if user_dict['login'] == user_login or issue_number in user_pr_numbers or issue_number in reviewed_pr_numbers:
                        user_id = user_dict['id']
                        if user_id not in users:
                            users[user_id] = User(**user_dict)
                        user = users[user_id]
        ...
        timeline_file = path / "canonical" / "timeline.jsonl"
        if timeline_file.exists():
            with open(timeline_file, 'r') as f:
                for line in f:
                    tl_dict = json.loads(line.strip())
                    # derive PR number from URL
                    url = tl_dict.get('url', '')
                    try:
                        pr_number = int(url.rstrip('/').split('/')[-2])
                    except Exception:
                        pr_number = tl_dict.get('comment_id') or tl_dict.get('id')
                        continue
                    ...
                    if actor_id not in users:
                        users[actor_id] = User(**actor_dict)
                    actor = users[actor_id]
                    if actor.login == user_login or pr_number in user_pr_numbers or pr_number in reviewed_pr_numbers:
                        evt = TimelineEvent(
                            id=tl_dict['id'],
        ...
        if files_file.exists():
            with open(files_file, 'r') as f:
                for line in f:
                    file_dict = json.loads(line.strip())
                    pr_number = file_dict.get('pull_request_number')
                    if pr_number in user_pr_numbers or pr_number in reviewed_pr_numbers:
