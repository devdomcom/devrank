...
def test_inline_comment_density():
    pr_author = make_user(id=1, login="alice")
    reviewer = make_user(id=2, login="bob")
    owner = make_user(id=3, login="org")
    repo = make_repo(id=1, name="repo", owner=owner)
    start = DEFAULT_START

    # PR1: bob reviews alice's PR with 2 inline comments
    pr1 = make_pr(1, pr_author, repo, base_time=start, created_delta_hours=0)
    ...
    # PR2: bob reviews alice's PR but no inline (general comment only)
    pr2 = make_pr(2, pr_author, repo, base_time=start, created_delta_hours=10)
    ...
    bundle = make_bundle(
        users=[pr_author, reviewer, owner],
        repositories=[repo],
    ...
    # Run metric for bob (the reviewer giving comments on others' PRs)
